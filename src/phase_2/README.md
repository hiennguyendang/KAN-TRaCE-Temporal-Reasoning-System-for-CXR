...
# 1) train (default imgsz=896; drop to 640 / smaller batch if you OOM)
...
python eval_yolo.py --weights /kaggle/working/runs/detect/det29/weights/best.pt --split test
...
- **Multi-session training**: checkpoints every `SAVE_PERIOD` epochs in
  `runs/detect/det29/weights/`. Keep that under `/kaggle/working` so it persists,
  then `--resume`.
